...
letters lastQuestion ls onclick = 
  table [style "scale" "200%", style "position" "relative", style "left" "50%", style "top" "50%", style "transform" "translate(-25%, -15%)" -- centering the whole thing
        ...
        ( tr [] (List.intersperse (th [style "font-size" "0.5cqh", style "width" "0.5cqh"] []) -- ruimte tussen letters
                  (List.map 
                    (\(i,l) -> let styles = [style "background-size" "100% 100%", style "font-size" "3cqh", style "opacity" "100%", style "height" "4cqh", style "width" "3cqh"]
                                   event = case onclick of
                                              Nothing -> []
                                              Just f -> [onClick (f i)] in
                      case l of
                        Opgezocht letter   -> th (style "background-color" "white" :: styles) [div event [text ((String.toUpper << String.slice 0 1) letter)]]
                        UitHetHoofd letter -> th (style "background-image" "url('images/uithethoofd.jpg')" :: styles) [div event [text ((String.toUpper << String.slice 0 1) letter)]]
